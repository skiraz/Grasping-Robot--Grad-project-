...
class PepperEnv(gymnasium.Env):    
    ...
    def _setupScene(self):
        ...
        if self.soda :
            ...
            p.changeVisualShape(self.soda, -1, textureUniqueId=texture_id)
    ...
    def _getState(self):
       ...
       #     self.episode_over = True
        ...
        if distance <= 0.28:
            self.dur.append(1)
            # reward += distance / (time.time() - self.episode_start_time)*1000 + 1
            # print(time.time() - self.episode_start_time)
           
        else:
            self.dur.append(0)

        success_cond = all(self.dur[-5:])
        if success_cond :
            self.episode_number += 1
            reward += distance * 100 / (time.time() - self.episode_start_time)*1000 + 1
            reward += 7000
            self.episode_over = True
            self.episode_failed = False
    ...
    def _getObservation(self):
        ...
        obs = {"kinematics":kinematics,"soda_can":SodaPos}
...
if __name__ == "__main__":
    ...
    for i in [0.001]:
        ...
        model.learn(total_timesteps=10000,tb_log_name=f"PPO_inverse_threshold_newlist_lr_{i}")
        


        model.save(os.path.join(env.models_dir,f"PPO_inverse_threshold_newlist_lr_{i}"))
        print("END OF LEARNING")
