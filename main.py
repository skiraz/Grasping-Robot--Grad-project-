...
class PepperEnv(gymnasium.Env):    
    ...
    def _getState(self, convergence_norm=0.15):
        ...
        #     self.episode_over = True
        if (time.time() - self.episode_start_time) > 4:
            reward += -1
            self.episode_over = True
            # self.episode_failed = True
        ...
        if self.episode_over:
            ...
            reward += - np.sqrt((SodaPos[0]-linkPos[0])**2+(SodaPos[1]-linkPos[1])**2+(SodaPos[2]+0.1-linkPos[2])**2)*3
            self.episode_reward += reward
    ...
    def _resetScene(self):
        ...
        self.soda_pos = [0.315, random_num, .96]
    ...
    def _getObservation(self):
        ...
        obs = {"kinematics":kinematics,"image":self.depth_image}
...
if __name__ == "__main__":
    ...
    model = PPO2.load(os.path.join(env.models_dir,"iter 5"),env=env)
    ...
    # it was 5
    model.learn(total_timesteps=20000 ,tb_log_name="ppo2_iter8")
     


    model.save(os.path.join(env.models_dir,"iter 9"))
