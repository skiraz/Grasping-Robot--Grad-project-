...
class PepperEnv(gymnasium.Env):    
    def __init__(self,gui = True):
        ...
        x_bounds = (0, 1)  # Minimum and maximum values for the x-coordinate
        y_bounds = (0, 1)  # Minimum and maximum values for the y-coordinate
        z_bounds = (-10, 10)  # Minimum and maximum values for the z-coordinate


        soda_can = spaces.Box(low=np.array([x_bounds[0], y_bounds[0]]),
        ...
        self.observation_space = spaces.Dict({
            ...
            "soda_can" :  soda_can,
            "image" :  spaces.Box(low=0, high=1, shape=(240, 320, 3))
        ...
        '''depth image'''
        # thread = threading.Thread(target=self.camera_view,args=(self.pepper,))
        # thread.start()
        time.sleep(3)
        self.contact_points_num =0  
        # thread = threading.Thread(target=self.contact_points)
        # thread.start()
    ...
    def _setupScene(self):
        ...
        if self.spawn_soda :
            ...
            p.changeVisualShape(self.soda, -1, textureUniqueId= p.loadTexture(r"D:\projects\graduation project\qi bullet simulation\pepsi.png"))
        ...
        if self.spawn_cup :
            self.cup = p.loadURDF(
                r"D:\projects\graduation project\qi bullet simulation\paper_cup.urdf",
                basePosition=self.cup_pos,
                globalScaling=0.6,
                physicsClientId=self.client)
            p.changeVisualShape(self.cup, -1, textureUniqueId=p.loadTexture(r"D:\projects\graduation project\qi bullet simulation\cup.jpg"))
    ...
    def _getState(self):
        ...
        """
        reward = 0.0
        # print(self.contact_points_num)
        self.contact_points_num = len(p.getContactPoints(self.robot_body_id,self.cup))
        ...
        if (time.time() - self.episode_start_time) > 5:
            ...
            print(self.contact_points_num)
            if not self.contact_points_num:
                self.episode_failed= True
            else :  
                reward  = self.contact_points_num*1000
        ...
        distance = np.sqrt(np.sqrt( (self.soda_pos[0]-linkPos[0])**2+(self.soda_pos[1]-linkPos[1])**2+(self.soda_pos[2]+0.04-linkPos[2])**2 ))
    ...
    def _resetScene(self):
        ...
        random_num = random.uniform(0.04, 0.25)

        self.soda_pos[1] = random_num
        self.cup_pos[1] = random_num
    ...
    def _printEpisodeSummary(self, info_dict={}):
        ...
        print("soda can coords" , self.x,self.y)
    ...
    def _getObservation(self):
        ...
        obs = {"kinematics":kinematics,"soda_can":[self.x,self.y],"image":self.img/255}
    ...
    def _setVelocities(self, angles, normalized_velocities):
        ...
        for angle, velocity in zip(angles, normalized_velocities):
            ...
            p.setJointMotorControl2(
                ...
                p.VELOCITY_CONTROL,
                targetVelocity=velocity*0.3,
                force=self.pepper.joint_dict[angle].getMaxEffort())

    def camera_view1(self,perspective):
    ...
    def camera_view(self,robot):
        ...
        for i in range(1):
            ...
            cv2.imwrite("rgb1.jpg",depth_image)
            # infer on a local image
            image_path = r"rgb1.jpg"
            # model.plot(image_path, confidence=40, overlap=30, save_path="predicted_image.jpg")
            self.model.predict(image_path=image_path, confidence=40, overlap=30).save(r"rgb_pred.jpg")
            pred = self.model.predict(image_path, confidence=40, overlap=30).json()

            self.img  = cv2.imread( r"rgb_pred.jpg")
            cv2.imshow("RGB", self.img)
            try :
                self.x = pred["predictions"][0]["x"]/320
                self.y = pred["predictions"][0]["y"]/240
            except : self.x,self.y = 0,0
    ...
    def reset(self):
        ...
        self.episode_reward = 0.0
        self.episode_steps = 0
        self.dur_all.append(sum(self.dur))
        self.contact_points_num =0
        self.x = 0 
        self.y = 0
        self.dur=[]
...
if __name__ == "__main__":
    ...
    env = PepperEnv(gui=1)
    step_time = env.metadata.get('timestep')
    ...
    model.learn(total_timesteps=1000 ,tb_log_name="ppo2_down_last_inverse_threshold_newlist_lr")
    
    # model.learn(total_timesteps=30000 ,tb_log_name="ppo2_iter5")


    model.save(os.path.join(env.models_dir,f"save_down_last_threshold_newlist_lr_{i}"))
    print("END OF LEARNING")

    a= pd.Series(env.dur_all)
    a.to_csv("all_dur.csv",index=False)
